...
signing flow therefore uses a development-only absolute-path exception and
registers `/` plus every currently mounted visible volume. A three-second timer
refreshes the volume set. It does not access an App Group container.
...
keeps their security scopes active. That distribution mode registers only the
authorized roots with `directoryURLs`. Path matching uses normalized path
...
The local ad hoc mode is deliberately separate. It registers the filesystem root
and mounted volumes and permits creation in any writable target returned by
Finder. The access mode is injected through the signed bundle's Info dictionary;
it is absent from normal distribution builds, which therefore remain
